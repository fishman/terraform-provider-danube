...
TF_PLUGIN_BINARY_NAME="terraform-provider-danube"
TF_PLUGIN_BINARY_PATH=".plugins/registry.terraform.io/fishman/danube/$TF_PLUGIN_VERSION/$(go env GOOS)_$(go env GOARCH)/"

if [ ! -f $TF_PLUGIN_BINARY_PATH ]; then
    mkdir -p $TF_PLUGIN_BINARY_PATH
fi

cp ./terraform-provider-danube $TF_PLUGIN_BINARY_PATH
